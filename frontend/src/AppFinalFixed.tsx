...
import { useLoggerFixed } from "@/hooks/useLoggerFixed";

const MAX_HISTORICAL_WORKFLOWS = 10; // Limit to prevent memory issues

export default function AppFinalFixed() {
  const scrollAreaRef = useRef<HTMLDivElement>(null);
  const [copiedMessageId, setCopiedMessageId] = useState<string | null>(null);
  const [isLogPanelOpen, setIsLogPanelOpen] = useState(false);
  const [completedWorkflows, setCompletedWorkflows] = useState<Record<string, any>>({});
  
  const { logs, logger, clearLogs } = useLoggerFixed();
  ...
  // Auto-scroll to bottom - optimized to only trigger on actual changes
  useEffect(() => {
    ...
    if (!isLoading && processedEvents.length > 0 && messages.length > 0) {
      ...
      if (lastMessage.role === 'assistant' && !completedWorkflows[lastMessage.id]) {
        setCompletedWorkflows(prev => {
          ...
          // Cleanup old workflows to prevent memory issues
          const keys = Object.keys(updated);
          if (keys.length > MAX_HISTORICAL_WORKFLOWS) {
            const sortedKeys = keys.sort((a, b) => {
              const aMsg = messages.find(m => m.id === a);
              const bMsg = messages.find(m => m.id === b);
              return (aMsg?.timestamp || 0) - (bMsg?.timestamp || 0);
            });
            const keysToRemove = sortedKeys.slice(0, keys.length - MAX_HISTORICAL_WORKFLOWS);
  ...
  }, [isLoading, processedEvents, messages, logger]);
  ...
  // Memoize connection indicator to prevent re-renders
  ...
  ), [connectionStatus.isConnected, isLogPanelOpen, logs.length]);
