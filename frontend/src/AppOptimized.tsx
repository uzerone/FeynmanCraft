...
import { Copy, CopyCheck, AlertCircle, User, Bot, Wifi, WifiOff, RefreshCw, Server, ChevronDown, ChevronUp } from "lucide-react";

// Optimized ProcessedEvent interface
...
// Optimized AgentWorkflow component without expand/collapse to reduce re-renders
function AgentWorkflow({ events, isLoading }: { events: ProcessedEvent[], isLoading: boolean }) {
  ...
  return (
    <div className="space-y-2">
      <h4 className="text-sm font-medium text-neutral-200 mb-3">
        🔄 Agent Workflow {!isLoading && events.length > 0 && '(Completed)'}
      </h4>
...
const MAX_HISTORICAL_WORKFLOWS = 5; // Limit stored workflows

export default function AppOptimized() {
  const scrollAreaRef = useRef<HTMLDivElement>(null);
  const [copiedMessageId, setCopiedMessageId] = useState<string | null>(null);
  const [historicalWorkflows, setHistoricalWorkflows] = useState<Record<string, ProcessedEvent[]>>({});
  ...
  useEffect(() => {
    if (!isLoading && processedEvents.length > 0 && messages.length > 0) {
      ...
      if (lastMessage.role === 'assistant' && !historicalWorkflows[lastMessage.id]) {
        setHistoricalWorkflows(prev => {
          const updated = {
            ...prev,
          ...
          if (keys.length > MAX_HISTORICAL_WORKFLOWS) {
            const keysToRemove = keys.slice(0, keys.length - MAX_HISTORICAL_WORKFLOWS);
              ...
              <div key={message.id || `msg-${index}`} className="space-y-3">
                ...
                >
                  {message.role === "user" ? (
                    ...
                    // AI message bubble with workflow
                    <div className="flex flex-col gap-2 w-full">
                        ...
                        <div className="relative group rounded-xl p-3 shadow-sm break-words bg-neutral-700 text-neutral-100 rounded-bl-none w-full">
                          <div className="prose prose-invert max-w-none">
                            ...
                            {message.content.startsWith('{') || message.content.startsWith('[') ? (
                              <pre className="overflow-x-auto text-xs bg-neutral-800 p-2 rounded">
                      ...
                      {/* Show historical workflow for this message */}
                      {historicalWorkflows[message.id] && (
                        <div className="flex items-start gap-3 ml-9">
                          <div className="relative group max-w-[85%] md:max-w-[80%] rounded-xl p-3 shadow-sm break-words bg-neutral-700 text-neutral-100 rounded-bl-none w-full">
                            <AgentWorkflow 
                              events={historicalWorkflows[message.id]} 
                              isLoading={false}
                            />
                          </div>
                        </div>
            ...
            <div className="flex items-start gap-3 mt-3">
              ...
              <div className="relative group max-w-[85%] md:max-w-[80%] rounded-xl p-3 shadow-sm break-words bg-neutral-700 text-neutral-100 rounded-bl-none w-full min-h-[56px]">
                {processedEvents.length > 0 ? (
                  <AgentWorkflow events={processedEvents} isLoading={true} />
                ) : (
