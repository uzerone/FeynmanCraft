...
def setup_module_paths():
    ...
    try:
        ...
        import mcp
        import pdg
        logging.info("Successfully imported MCP and PDG modules")
    except ImportError as e:
        logging.error(f"Failed to import required modules: {e}")
        # Fallback: try to find using pip
        try:
            result = subprocess.run(['pip', 'show', 'mcp'], 
            ...
            result = subprocess.run(['pip', 'show', 'pdg'], 
                                  capture_output=True, text=True, check=True)
            for line in result.stdout.split('\n'):
                if line.startswith('Location:'):
            ...
            logging.warning(f"Could not automatically find module paths: {fallback_e}")
...
async def search_particle(arguments: dict) -> list[types.TextContent]:
    ...
    try:
        # Try to import PDG module directly
        try:
            import pdg
        except ImportError as e:
            return [types.TextContent(type="text", text=f"Error: PDG package not available. Import error: {str(e)}. Please install with: pip install pdg")]
